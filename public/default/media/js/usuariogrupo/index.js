	...
	$('.cadastro-rapido').click(function(){
		var $modal = $('.forget-modal-all');
 		...
 		$('body').modalmanager('loading');
     		$modal.load(pathSite+'/default/usuariogrupo/cadastro-rapido/type/json', function( response, status, xhr ){
...
function operateFormatter(value, row, index) {
    return [
		...
		'</a></div>',
        '<div class="col-md-4 col-xs-12 col-sm-4" align="center"><a data-toggle="tooltip" href="#" class="btn btn-block btn-sm btn-primary editFast" title="Edição Rápida">',
...
window.operateEvents = {
		'click .editar': function (e, value, row, index) {
    		window.location.href=pathSite+'/default/usuariogrupo/editar-form/id/'+row.id;
		},
        'click .editFast': function (e, value, row, index) {
        	...
        	// create the backdrop and wait for next modal to be triggered
        	$('body').modalmanager('loading');
        	$modal.load(pathSite+'/default/usuariogrupo/editar-rapido/id/'+row.id+"/type/json", function( response, status, xhr ){
        		var resposta = xhr.responseText;
         		if(resposta.indexOf("situacao") == -1){	
         			$modal.modal();
        ...
        'click .remove': function (e, value, row, index) {
            ...
            if(r) {
            	$.ajax({
					url: pathSite+'/default/usuariogrupo/excluir/id/'+row.id+"/type/json",
					dataType: 'json',
					type: 'POST',
					data: {
						id: row.id
